...
<main class="pages-pays" style="background: <?php echo esc_attr($pays_background_color); ?>;">
    ...
    <div class="contenu-pays" style="background: <?php echo esc_attr($pays_background_color); ?>;">
        ...
        <div class="pays-galerie">
            <div class="pays-galerie__wrapper">
                <?php
                $nombre_images = get_theme_mod('pays_galerie_nombre_images', 3);
                for ($i = 1; $i <= $nombre_images; $i++) {
                    $img = get_theme_mod("pays_galerie_image_$i");
                    if ($img) {
                        echo '<figure class="pays-galerie__item"><img src="' . esc_url($img) . '" alt="Image galerie ' . $i . '"></figure>';
                    }
                }
                ?>
            </div>
